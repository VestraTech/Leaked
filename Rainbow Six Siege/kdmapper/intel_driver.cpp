...
bool intel_driver::IsRunning() {
	const HANDLE file_handle = CreateFileW(XorStr(L"\\\\.\\Nal").c_str(), FILE_ANY_ACCESS, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
HANDLE intel_driver::Load() {
	...
	HANDLE result = CreateFileW(XorStr(L"\\\\.\\Nal").c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
{
	static uint64_t kernel_MmAllocatePagesForMdl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, XorStr("MmAllocatePagesForMdl").c_str());
...
{
	static uint64_t kernel_MmMapLockedPagesSpecifyCache = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, XorStr("MmMapLockedPagesSpecifyCache").c_str());
...
{
	static uint64_t kernel_MmProtectMdlSystemAddress = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, XorStr("MmProtectMdlSystemAddress").c_str());
...
{
	static uint64_t kernel_MmUnmapLockedPages = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, XorStr("MmUnmapLockedPages").c_str());
...
{
	static uint64_t kernel_MmFreePagesFromMdl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, XorStr("MmFreePagesFromMdl").c_str());
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size) {
	...
	static uint64_t kernel_ExAllocatePool = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, XorStr("ExAllocatePoolWithTag").c_str());
...
bool intel_driver::FreePool(HANDLE device_handle, uint64_t address) {
	...
	static uint64_t kernel_ExFreePool = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, XorStr("ExFreePool").c_str());
...
bool intel_driver::ExAcquireResourceExclusiveLite(HANDLE device_handle, PVOID Resource, BOOLEAN wait) {
	...
	static uint64_t kernel_ExAcquireResourceExclusiveLite = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, XorStr("ExAcquireResourceExclusiveLite").c_str());
...
bool intel_driver::ExReleaseResourceLite(HANDLE device_handle, PVOID Resource) {
	...
	static uint64_t kernel_ExReleaseResourceLite = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, XorStr("ExReleaseResourceLite").c_str());
...
BOOLEAN intel_driver::RtlDeleteElementGenericTableAvl(HANDLE device_handle, PVOID Table, PVOID Buffer) {
	...
	static uint64_t kernel_RtlDeleteElementGenericTableAvl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, XorStr("RtlDeleteElementGenericTableAvl").c_str());
...
PVOID intel_driver::RtlLookupElementGenericTableAvl(HANDLE device_handle, PRTL_AVL_TABLE Table, PVOID Buffer) {
	...
	static uint64_t kernel_RtlDeleteElementGenericTableAvl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, XorStr("RtlLookupElementGenericTableAvl").c_str());
...
bool intel_driver::ClearPiDDBCacheTable(HANDLE device_handle) { 
	...
	PiDDBLockPtr = FindPatternInSectionAtKernel(device_handle, (char*)XorStr("PAGE").c_str(), intel_driver::ntoskrnlAddr, (PUCHAR)XorStr("\x8B\xD8\x85\xC0\x0F\x88\x00\x00\x00\x00\x65\x48\x8B\x04\x25\x00\x00\x00\x00\x66\xFF\x88\x00\x00\x00\x00\xB2\x01\x48\x8D\x0D\x00\x00\x00\x00\xE8\x00\x00\x00\x00\x4C\x8B\x00\x24").c_str(), (char*)XorStr("xxxxxx????xxxxx????xxx????xxxxx????x????xx?x").c_str()); // 8B D8 85 C0 0F 88 ? ? ? ? 65 48 8B 04 25 ? ? ? ? 66 FF 88 ? ? ? ? B2 01 48 8D 0D ? ? ? ? E8 ? ? ? ? 4C 8B ? 24 update for build 22000.132
	PiDDBCacheTablePtr = FindPatternInSectionAtKernel(device_handle, (char*)XorStr("PAGE").c_str(), intel_driver::ntoskrnlAddr, (PUCHAR)XorStr("\x66\x03\xD2\x48\x8D\x0D").c_str(), (char*)XorStr("xxxxxx").c_str()); // 66 03 D2 48 8D 0D
	...
	if (PiDDBLockPtr == NULL) { // PiDDBLock pattern changes a lot from version 1607 of windows and we will need a second pattern if we want to keep simple as posible
		PiDDBLockPtr = FindPatternInSectionAtKernel(device_handle, (char*)XorStr("PAGE").c_str(), intel_driver::ntoskrnlAddr, (PUCHAR)XorStr("\x48\x8B\x0D\x00\x00\x00\x00\x48\x85\xC9\x0F\x85\x00\x00\x00\x00\x48\x8D\x0D\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8").c_str(), (char*)XorStr("xxx????xxxxx????xxx????x????x").c_str()); // 48 8B 0D ? ? ? ? 48 85 C9 0F 85 ? ? ? ? 48 8D 0D ? ? ? ? E8 ? ? ? ? E8 build 22449+ (pattern can be improved but just fine for now)
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	uint64_t ci = utils::GetKernelModuleAddress(XorStr("ci.dll").c_str());
	if (!ci) {
		return false;
	}

	//Thanks @KDIo3 and @Swiftik from UnknownCheats
	auto sig = FindPatternInSectionAtKernel(device_handle, (char*)XorStr("PAGE").c_str(), ci, PUCHAR(XorStr("\x48\x8B\x1D\x00\x00\x00\x00\xEB\x00\xF7\x43\x40\x00\x20\x00\x00").c_str()), (char*)XorStr("xxx????x?xxxxxxx").c_str());
	if (!sig) {
		return false;
	}
	auto sig2 = FindPatternAtKernel(device_handle, (uintptr_t)sig - 50, 50, PUCHAR(XorStr("\x48\x8D\x0D").c_str()), (char*)XorStr("xxx").c_str());
	if (!sig2) {
		return false;
	}
