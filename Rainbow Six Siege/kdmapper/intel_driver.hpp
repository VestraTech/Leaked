...
namespace intel_driver
{
	...
	bool CallKernelFunction(HANDLE device_handle, T* out_result, uint64_t kernel_function_address, const A ...arguments) {
		...
		const auto NtAddAtom = reinterpret_cast<void*>(GetProcAddress(ntdll, XorStr("NtAddAtom").c_str()));
		if (!NtAddAtom)
		{
			return false;
		}
		...
		*(uint64_t*)&kernel_injected_jmp[2] = kernel_function_address;
		...
		static uint64_t kernel_NtAddAtom = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, XorStr("NtAddAtom").c_str());
		if (!kernel_NtAddAtom) {
			return false;
		}
